...
if ( ! function_exists( 'aravan_enqueue_google_fonts' ) ) {
	add_action( 'wp_enqueue_scripts', 'aravan_enqueue_google_fonts', 0 );
	/**
	 * Add Google Fonts to wp_head if needed.
	 *
	 */
	function aravan_enqueue_google_fonts() {
		...
		$not_google = str_replace( ' ', '+', aravan_typography_default_fonts() );
		...
		if ( ! empty( $font_settings ) ) {
			...
			foreach ( $font_settings as $key ) {
				...
				if ( ! isset( $aravan_settings[$key] ) ) {
					continue;
				}

				// If our value is still using the old format, fix it
				if ( strpos( $aravan_settings[$key], ':' ) !== false ) {
					$aravan_settings[$key] = current( explode( ':', $aravan_settings[$key] ) );
				}

				// Replace the spaces in the names with a plus
				$value = str_replace( ' ', '+', $aravan_settings[$key] );

				// Grab the variants using the plain name
				$variants = aravan_get_google_font_variants( $aravan_settings[$key], $key );
		...
		$google_fonts = apply_filters( 'aravan_typography_google_fonts', $google_fonts );

		// Get the subset
		$subset = apply_filters( 'aravan_fonts_subset','' );
		...
		$default_fonts = ARAVAN_DEFAULT_FONTS;

		// Create our URL using the arguments
		$fonts_url = add_query_arg( $font_args, '//fonts.googleapis.com/css' );

		// Enqueue our fonts
		wp_enqueue_style('aravan-default-fonts', $default_fonts, array(), null, 'all' );
		if ( $google_fonts ) {
			wp_enqueue_style('aravan-fonts', $fonts_url, array(), null, 'all' );
		}
	}
}

if ( ! function_exists( 'aravan_default_fonts_customize_register' ) ) {
	add_action( 'customize_register', 'aravan_default_fonts_customize_register' );
	/**
	 * Build our Typography options
	 *
	 */
	function aravan_default_fonts_customize_register( $wp_customize ) {
		if ( function_exists( 'aravan_fonts_customize_register' ) ) {
			// Bail if Aravan Premium is active
			return;
		}
		...
		$defaults = aravan_get_default_fonts();

		if ( method_exists( $wp_customize,'register_control_type' ) ) {
			$wp_customize->register_control_type( 'Aravan_Typography_Customize_Control' );
		...
		$wp_customize->add_setting(
			...
			array(
				...
				'sanitize_callback' => 'aravan_sanitize_variants'
			)
		);

		$wp_customize->add_setting(
			'aravan_settings[body_font_weight]',
			array(
		...
		$wp_customize->add_setting(
			'aravan_settings[body_font_transform]',
			array(
		...
		$wp_customize->add_control(
			new Aravan_Typography_Customize_Control(
				$wp_customize,
				'body_typography',
				array(
					'section' => 'font_section',
					'priority' => 1,
					'label' => __( 'Text Font', 'aravan' ),
					'settings' => array(
						'family' => 'aravan_settings[font_body]',
						'variant' => 'font_body_variants',
						'category' => 'font_body_category',
						'weight' => 'aravan_settings[body_font_weight]',
						'transform' => 'aravan_settings[body_font_transform]',
					),
				)
			)
		);
		...
		$wp_customize->add_setting(
			'aravan_settings[body_font_size]',
			array(
		...
		$wp_customize->add_control(
			new Aravan_Range_Slider_Control(
				...
				array(
					...
					'description' => __( 'Font size', 'aravan' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'aravan_settings[body_font_size]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 6,
		...
		$wp_customize->add_setting(
			'aravan_settings[body_line_height]',
			array(
				'default' => $defaults['body_line_height'],
				'type' => 'option',
				'sanitize_callback' => 'aravan_sanitize_decimal_integer',
				'transport' => 'postMessage'
			)
		);
		...
		$wp_customize->add_control(
			new Aravan_Range_Slider_Control(
				...
				array(
					'type' => 'aravan-range-slider',
					'description' => __( 'Line height', 'aravan' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'aravan_settings[body_line_height]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 1,
		...
		$wp_customize->add_setting(
			'aravan_settings[paragraph_margin]',
			array(
		...
		$wp_customize->add_control(
			new Aravan_Range_Slider_Control(
				...
				array(
					'type' => 'aravan-range-slider',
					'description' => __( 'Paragraph margin', 'aravan' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'aravan_settings[paragraph_margin]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 0,
...
if ( ! function_exists( 'aravan_get_all_google_fonts' ) ) {
	/**
	 ...
	 */
	function aravan_get_all_google_fonts( $amount = 'all' ) {
		...
		if ( apply_filters( 'aravan_alphabetize_google_fonts', true ) ) {
			asort( $fonts );
		}

		// Filter to allow us to modify the fonts array
		return apply_filters( 'aravan_google_fonts_array', $fonts );
	}
}

if ( ! function_exists( 'aravan_get_all_google_fonts_ajax' ) ) {
	add_action( 'wp_ajax_aravan_get_all_google_fonts_ajax', 'aravan_get_all_google_fonts_ajax' );
	/**
	 * Return an array of all of our Google Fonts.
	 *
	 *
	 */
	function aravan_get_all_google_fonts_ajax() {
		// Bail if the nonce doesn't check out
		if ( ! isset( $_POST[ 'aravan_customize_nonce' ] ) || ! wp_verify_nonce( sanitize_key( $_POST[ 'aravan_customize_nonce' ] ), 'aravan_customize_nonce' ) ) {
			wp_die();
		}

		// Do another nonce check
		check_ajax_referer( 'aravan_customize_nonce', 'aravan_customize_nonce' );
		...
		$fonts = aravan_get_all_google_fonts();
...
if ( ! function_exists( 'aravan_get_google_font_variants' ) ) {
	/**
	 * Wrapper function to find variants for chosen Google Fonts
	 * Example: aravan_get_google_font_variation( 'Open Sans' )
	 *
	 */
	function aravan_get_google_font_variants( $font, $key = '' ) {
		// Don't need variants if we're using a system font
		if ( in_array( $font, aravan_typography_default_fonts() ) ) {
			return;
		}
		...
		if ( $variants ) {
			...
			return implode(',', apply_filters( 'aravan_typography_variants', $output ) );
		}
	}
}

if ( ! function_exists( 'aravan_get_google_font_category' ) ) {
	/**
	 * Wrapper function to find the category for chosen Google Font
	 * Example: aravan_get_google_font_category( 'Open Sans' )
	 *
	 *
	 *
	 ...
	 */
	function aravan_get_google_font_category( $font, $key = '' ) {
...
if ( ! function_exists( 'aravan_get_font_family_css' ) ) {
	/**
	 ...
	 */
	function aravan_get_font_family_css( $font, $settings, $default ) {
		$aravan_settings = wp_parse_args(
		...
		$no_quotes = array(
			...
			apply_filters( 'aravan_typography_system_stack', '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' )
		);

		// Get our font
		$font_family = $aravan_settings[ $font ];

		if ( 'System Stack' == $font_family ) {
			$font_family = apply_filters( 'aravan_typography_system_stack', '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' );
		}
		...
		if ( in_array( $font_family, $no_quotes ) ) {
			...
			$wrapper_start = '"';
			$wrapper_end = '"' . aravan_get_google_font_category( $font_family, $font );
		}
...
if ( ! function_exists( 'aravan_add_to_font_customizer_list' ) ) {
	add_filter( 'aravan_typography_customize_list', 'aravan_add_to_font_customizer_list' );
	/**
	 ...
	 *
	 */
	function aravan_add_to_font_customizer_list( $fonts ) {
		...
		$all_fonts = aravan_get_all_google_fonts();
		$select_fonts = aravan_get_all_google_fonts( apply_filters( 'aravan_number_of_fonts', 200 ) );

		foreach ( $font_settings as $setting ) {
			// If we don't have a setting, keep going
			if ( ! isset( $aravan_settings[ $setting ] ) ) {
				continue;
			}

			$id = strtolower( str_replace( ' ', '_', $aravan_settings[ $setting ] ) );

			if ( array_key_exists( $id, $select_fonts ) || in_array( $aravan_settings[ $setting ], aravan_typography_default_fonts() ) ) {
				continue;
			}

			$fonts[ strtolower( str_replace( ' ', '_', $aravan_settings[ $setting ] ) ) ] = array(
				'name' => $aravan_settings[ $setting ],
		...
		if ( function_exists( 'aravan_secondary_nav_get_defaults' ) ) {
			...
			if ( ! array_key_exists( $secondary_nav_id, $select_fonts ) && ! in_array( $secondary_nav_settings[ 'font_secondary_navigation' ], aravan_typography_default_fonts() ) ) {
